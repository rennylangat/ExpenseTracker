...
public class Splash extends SplashView.SplashAdLoadListener {
    ...
    public Splash(int id, Activity activity, MethodChannel channel) {
        this.id = id;
        this.activity = activity;
        this.channel = channel;
        this.status = Constants.AdStatus.CREATED;
        this.topMargin = 0.0;
        this.adType = SplashAdType.above;
    ...
    public static void preloadAd(Activity activity, String adSlotId, int orientation, Map<String, Object> adParamMap, MethodChannel.Result result) {
        ...
        try {
            ...
            result.error(Constants.ErrorCodes.LOAD_FAILED, "Preload failed for Splash ad", "");
        }
    }
    ...
    public void loadAd(
        ...
        if (type != null) {
            adType = SplashAdType.valueOf(type);
        }
        if (margin != null) {
            topMargin = margin;
        }

        adView = new SplashView(activity);
        ...
        adView.setAdDisplayListener(new SplashAdDisplayListener() {
            @Override
            public void onAdShowed() {
                Log.i(TAG, "onAdShowed");
                channel.invokeMethod("onSplashAdShowed", ToMap.argsToMap(id));
            }

            @Override
            public void onAdClick() {
                Log.i(TAG, "onAdClick");
                channel.invokeMethod("onSplashAdClick", ToMap.argsToMap(id));
            }
        });

        adView.load(adSlotId, orientation, adParam, this);
    }
    ...
    public void onAdFailedToLoad(int errorCode) {
        ...
        channel.invokeMethod("onSplashAdFailed", ToMap.argsToMap(id, "errorCode", errorCode));
    }
    ...
    public void onAdLoaded() {
        ...
        show();
        channel.invokeMethod("onSplashAdLoaded", ToMap.argsToMap(id));
    }

    @Override
    public void onAdDismissed() {
        Log.i(TAG, "onAdDismissed");
        channel.invokeMethod("onSplashAdDismissed", ToMap.argsToMap(id));
    }
