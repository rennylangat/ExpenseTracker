...
import com.huawei.hms.flutter.ads.factory.VideoConfigurationBuilderFactory;
...
public class NativeAdConfigurationFactory {
    private static final String TAG = "NativeAdConfigurationBuilderFactory";
    private final Map<String, Object> configurationArgs;

    public NativeAdConfigurationFactory(Map<String, Object> configurationArgs) {
        this.configurationArgs = configurationArgs;
    }

    public NativeAdConfiguration.Builder createNativeAdConfiguration() {
        NativeAdConfiguration.Builder builder = new NativeAdConfiguration.Builder();
        if (configurationArgs == null) {
            return builder;
        }

        Map<String, Object> adSizeMap = ToMap.objectToMap(configurationArgs.get("adSize"));
        ...
        Map<String, Object> videoConfigurationMap = ToMap.objectToMap(configurationArgs.get("videoConfiguration"));
        if (!videoConfigurationMap.isEmpty()) {
            VideoConfigurationBuilderFactory videoConFactory = new VideoConfigurationBuilderFactory(videoConfigurationMap);
            VideoConfiguration videoConfiguration = videoConFactory.createVideoConfigurationBuilder().build();
