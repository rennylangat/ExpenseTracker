...
public class NativeStyles {
    ...
    public NativeStyles build(Map<String, Object> args) {
        ...
        Map<String, Object> flagMap = ToMap.objectToMap(args.get("flag"));
        if (!flagMap.isEmpty()) {
            buildNativeStyle(styles.flag, flagMap);
        }

        Map<String, Object> sourceMap = ToMap.objectToMap(args.get("source"));
        if (!sourceMap.isEmpty()) {
            buildNativeStyle(styles.source, sourceMap);
        }

        Map<String, Object> titleMap = ToMap.objectToMap(args.get("title"));
        if (!titleMap.isEmpty()) {
            buildNativeStyle(styles.title, titleMap);
        }

        Map<String, Object> descriptionMap = ToMap.objectToMap(args.get("description"));
        if (!descriptionMap.isEmpty()) {
            buildNativeStyle(styles.description, descriptionMap);
        }

        Map<String, Object> callToActionMap = ToMap.objectToMap(args.get("callToAction"));
