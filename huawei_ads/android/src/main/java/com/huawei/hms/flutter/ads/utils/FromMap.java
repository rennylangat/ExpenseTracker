...
public class FromMap {
    ...
    public static String toString(String key, Object value) {
        if (!(value instanceof String) || ((String) value).isEmpty()) {
            Log.w(TAG, key + ": Non-empty String expected");
            return null;
        }
        return (String) value;
    }
            ...
            if (objArr != null) {
                for (Object o : objArr) {
                    if (o instanceof String) {
                        ...
                        Log.w(TAG, key + ": String expected");
                    }
                }
            }
        } else {
            Log.w(TAG, key + ": list expected");
        }
        return arrList;
    }
    ...
    public static Boolean toBoolean(String key, Object value) {
        if (!(value instanceof Boolean)) {
            Log.w(TAG, key + ": boolean expected");
    ...
    private static boolean[] toBooleanArray(String key, Object value) {
        ...
        Log.w(TAG, key + ": expected a list");
        return new boolean[0];
    }

    public static Long toLong(String key, Object value) {
        if (!(value instanceof Integer)) {
            Log.w(TAG, key + ": long expected");
            return null;
        }
    ...
    public static Integer toInteger(String key, Object value) {
        if (!(value instanceof Integer)) {
            Log.w(TAG, key + ": integer expected");
            return null;
        }
    ...
    public static Bundle toBundle(Object args) {
        Map<String, Object> bundleMap = ToMap.objectToMap(args);
        Bundle bundle = new Bundle();
        if (bundleMap.isEmpty()) {
            return null;
        }
        for (Map.Entry<String, Object> entry : bundleMap.entrySet()) {
            Map<String, Object> item = ToMap.objectToMap(entry.getValue());
            if (item.isEmpty()) {
                continue;
            }
